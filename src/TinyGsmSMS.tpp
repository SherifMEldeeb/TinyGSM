...
public:
    ...
    enum SMSMode {
        E_TEXT_MODE, E_PDU_MODE
    };

    std::string getSMSAsJson(SMSReadStatus status, SMSMode mode = E_PDU_MODE) {
        thisModem().sendAT(GF("+CMGF="), GF(mode));
        thisModem().waitResponse();
        thisModem().sendAT(GF("+CMGL="), GF(status));
        if (thisModem().waitResponse() != 1) { DBG("No SMS Response");return ""; }
        thisModem().streamSkipUntil(':'); /* Skip mode and format */
        thisModem().streamSkipUntil('"'); /* Skip mode and format */
        std::string sender = thisModem().stream.readStringUntil('"').c_str();
        thisModem().streamSkipUntil(','); /* Skip mode and format */
        unsigned int i = thisModem().stream.readStringUntil('\n').toInt();
        thisModem().streamSkipUntil('\n'); /* Skip mode and format */
        char body[i + 1];
        memset(body, 0, i + 1);
        thisModem().stream.readBytes(body, i);
        DBG(sender.c_str());
        DBG(body);
        return body;
    }
